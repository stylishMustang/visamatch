  ...
  for (const job of jobs) {
    ...
    try {
      ...
      if (error) {
        ...
        console.log(
          `✅ Inserted: ${job.job_title} @ ${job.employer_name} → ${visa_verified ? visa_type : '🎓 OPT fallback'}`,
        );
      }
    } catch (err) {
...
if (require.main === module) {
  const inputRole = process.argv[2] || 'biomedical engineer internship';
  matchAndInsertJobs(inputRole).catch((err) => {
